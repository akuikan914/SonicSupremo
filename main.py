...
def cmd_deposit(args: argparse.Namespace) -> int:
    ...
    pod_id = getattr(args, "pod_id", None)
    amount_wei = getattr(args, "amount_wei", None)
    if pod_id is None or amount_wei is None:
        print("Error: --pod-id and --amount-wei required", file=sys.stderr)
        return 1
    try:
        pod_id = int(pod_id)
        amount_wei = parse_wei(str(amount_wei))
...
def cmd_register_pod(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if receipt["status"] != 1:
            ...
            return 1
        next_id = contract.functions.getNextPodId().call()
        print("Pod registered. New pod ID:", next_id)
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        acct = get_signer_account(w3, pk)
        contract = get_contract(w3, contract_addr)
        tx = contract.functions.pause().build_transaction({"from": acct.address, "gas": 100000})
        tx["gas"] = w3.eth.estimate_gas(tx)
        signed = acct.sign_transaction(tx)
        tx_hash = w3.eth.send_raw_transaction(signed.raw_transaction)
        receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
        if receipt["status"] != 1:
            print("Transaction failed", file=sys.stderr)
            return 1
        print("Protocol paused. Tx:", tx_hash.hex())
    except Exception as e:
        print("Error:", e, file=sys.stderr)
        return 1
    return 0
...
def cmd_unpause(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        acct = get_signer_account(w3, pk)
        contract = get_contract(w3, contract_addr)
        tx = contract.functions.unpause().build_transaction({"from": acct.address, "gas": 100000})
        tx["gas"] = w3.eth.estimate_gas(tx)
        signed = acct.sign_transaction(tx)
        tx_hash = w3.eth.send_raw_transaction(signed.raw_transaction)
        receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
...
def cmd_withdraw_batch(args: argparse.Namespace) -> int:
    ...
    if not pk:
        ...
        return 1
    pod_id = getattr(args, "pod_id", None)
    indices_str = getattr(args, "indices", None)
    if pod_id is None or not indices_str:
        print("Error: --pod-id and --indices (comma-separated) required", file=sys.stderr)
        return 1
    try:
        pod_id = int(pod_id)
        indices = [int(x.strip()) for x in str(indices_str).split(",")]
        validate_pod_id(pod_id)
        w3 = get_w3(rpc)
        acct = get_signer_account(w3, pk)
        contract = get_contract(w3, contract_addr)
        tx = contract.functions.withdrawBatch(pod_id, indices).build_transaction({
            "from": acct.address,
            "gas": 300000,
        })
        tx["gas"] = w3.eth.estimate_gas(tx)
        ...
        if receipt["status"] != 1:
            print("Transaction failed", file=sys.stderr)
            return 1
        print("Batch withdrawal successful. Tx:", tx_hash.hex())
    except Exception as e:
        print("Error:", e, file=sys.stderr)
        return 1
    return 0

def cmd_claim_reward_batch(args: argparse.Namespace) -> int:
    rpc = args.rpc_url or load_config().get("rpc_url", DEFAULT_RPC_URL)
    contract_addr = args.contract or load_config().get("contract", DEFAULT_CONTRACT)
    if not contract_addr:
        print("Error: --contract or config required", file=sys.stderr)
    ...
    try:
        ...
        acct = get_signer_account(w3, pk)
        contract = get_contract(w3, contract_addr)
        tx = contract.functions.claimRewardBatch(pod_id, indices).build_transaction({
            "from": acct.address,
            "gas": 250000,
        })
        tx["gas"] = w3.eth.estimate_gas(tx)
...
def main() -> int:
    ...
    p_unpause = sub.add_parser("unpause", help="[Guardian] Unpause protocol")
    p_unpause.add_argument("--private-key", required=True)
    p_unpause.set_defaults(func=cmd_unpause)

    # set-fee
    p_fee = sub.add_parser("set-fee", help="[Guardian] Set protocol fee (bps)")
    p_fee.add_argument("--private-key", required=True)
    p_fee.add_argument("--fee-bps", required=True)
    p_fee.set_defaults(func=cmd_set_fee)

    # quote / simulate
    p_quote = sub.add_parser("quote", help="Quote fee and net for a deposit amount")
    p_quote.add_argument("--amount-wei", required=True)
    p_quote.set_defaults(func=cmd_quote)
    p_sim = sub.add_parser("simulate", help="Simulate deposit into a pod")
    p_sim.add_argument("--pod-id", required=True)
    p_sim.add_argument("--amount-wei", required=True)
    p_sim.set_defaults(func=cmd_simulate)

    # pod-info
    p_pi = sub.add_parser("pod-info", help="Info for one pod")
    p_pi.add_argument("--pod-id", required=True)
    p_pi.set_defaults(func=cmd_pod_info)

    # protocol-health, available-pods, validate-deposit, user-global-stats, withdrawable
    p_health = sub.add_parser("protocol-health", help="Protocol balance vs reserved health")
    p_health.set_defaults(func=cmd_protocol_health)
    p_avail = sub.add_parser("available-pods", help="Pods with capacity remaining")
    p_avail.set_defaults(func=cmd_available_pods)
    p_val = sub.add_parser("validate-deposit", help="Validate deposit params (pod-id, amount)")
    p_val.add_argument("--pod-id", required=True)
    p_val.add_argument("--amount-wei", required=True)
    p_val.set_defaults(func=cmd_validate_deposit)
    p_ugs = sub.add_parser("user-global-stats", help="User total principal and claimable across all pods")
    p_ugs.add_argument("--address", required=True)
    p_ugs.set_defaults(func=cmd_user_global_stats)
    p_wdraw = sub.add_parser("withdrawable", help="List withdrawable positions for an address")
    p_wdraw.add_argument("--address", required=True)
    p_wdraw.set_defaults(func=cmd_withdrawable)
    sub.add_parser("diagnostics", help="Run full diagnostics (many view calls)").set_defaults(func=cmd_diagnostics)
    sub.add_parser("summary", help="Dashboard + health + pods in one").set_defaults(func=cmd_summary)
    p_ur = sub.add_parser("user-report", help="Structured report for one address")
    p_ur.add_argument("--address", required=True)
    p_ur.set_defaults(func=cmd_user_report)
    p_er = sub.add_parser("export-report", help="Export user report as JSON to file")
    p_er.add_argument("--address", required=True)
    p_er.add_argument("--output", required=True, help="Output file path")
    p_er.set_defaults(func=cmd_export_report)
    p_ce = sub.add_parser("check-eligibility", help="Check if deposit would be accepted")
    p_ce.add_argument("--pod-id", required=True)
    p_ce.add_argument("--amount-wei", required=True)
    p_ce.set_defaults(func=cmd_check_eligibility)
    sub.add_parser("help-all", help="List all commands with short descriptions").set_defaults(func=cmd_help_all)
    sub.add_parser("events-help", help="List SonicSaver contract events").set_defaults(func=cmd_events_help)
    sub.add_parser("presets", help="Show pod presets for register-pod").set_defaults(func=cmd_presets)
    sub.add_parser("chain-id", help="Show current chain ID").set_defaults(func=cmd_chain_id)
    sub.add_parser("block-info", help="Show latest block number and timestamp").set_defaults(func=cmd_block_info)
    p_snap = sub.add_parser("protocol-snapshot", help="Full protocol snapshot (JSON); optional --output file")
    p_snap.add_argument("--output", help="Write JSON to file")
    p_snap.set_defaults(func=cmd_protocol_snapshot)

    # version / constants / interactive
    sub.add_parser("version", help="Show version").set_defaults(func=cmd_version)
    p_const = sub.add_parser("constants", help="Show contract constants")
